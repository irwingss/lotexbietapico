...
generar_texto_conclusiones <- function(muestra, celdas, locaciones, umbral, 
  ...
  grillas_no_impactadas <- muestra %>% filter(TPH <= umbral)
  n_grillas_limpias <- nrow(grillas_no_impactadas)
  locaciones_con_grillas_limpias <- sort(unique(grillas_no_impactadas$LOCACION))
  n_locs_con_limpias <- length(locaciones_con_grillas_limpias)
  
  # Grillas que SÍ superan umbral
  grillas_impactadas <- muestra %>% filter(TPH > umbral)
  n_grillas_impactadas <- nrow(grillas_impactadas)
  locaciones_con_grillas_impactadas <- sort(unique(grillas_impactadas$LOCACION))
  n_locs_con_impactadas <- length(locaciones_con_grillas_impactadas)
  
  # ============ GENERAR DETALLE DE PUNTOS impactados POR LOCACIÓN ============
  detalle_puntos_impactados <- ""
  if (n_grillas_impactadas > 0) {
    # Agrupar por locación
    puntos_por_loc <- grillas_impactadas %>%
      group_by(LOCACION) %>%
    ...
    if (length(detalle_por_tipo) == 1) {
      detalle_puntos_impactados <- detalle_por_tipo[[1]]
    } else if (length(detalle_por_tipo) == 2) {
      detalle_puntos_impactados <- paste(detalle_por_tipo, collapse = "; y ")
    } else {
      detalle_puntos_impactados <- paste0(
  ...
  # Puntos que pertenecen a locaciones impactadas (excluidos de grillas individuales)
  puntos_en_locs_impactadas <- ""
  if (n_locaciones_impactadas > 0) {
    locs_cont_vec <- locaciones_impactadas$LOCACION
    puntos_excluidos <- grillas_impactadas %>%
    ...
    if (nrow(puntos_excluidos) > 0) {
      ...
      puntos_en_locs_impactadas <- paste(detalles_excl, collapse = ", ")
    }
  }
  ...
  locs_impactadas_nombres <- "[NINGUNA]"
  areas_locaciones_individuales <- list()
  area_locaciones <- 0
  
  if (n_locaciones_impactadas > 0) {
    locs_impactadas_vec <- locaciones_impactadas$LOCACION
    ...
    if (!is.null(shp_celdas) && !is.null(col_area_celdas)) {
      ...
      if (!is.na(col_loc_shp) && col_area_celdas %in% cols_shp) {
        ...
        for (loc in locs_impactadas_vec) {
        ...
        if (length(areas_locaciones_individuales) > 0) {
          ...
          locs_impactadas_nombres <- paste(lista_con_areas, collapse = ", ")
        }
      }
    }
    ...
    if (area_locaciones == 0) {
      area_locaciones <- n_locaciones_impactadas * 500
      # Lista sin áreas individuales
      locs_impactadas_nombres <- paste(sort(locs_impactadas_vec), collapse = ", ")
    }
  }
  ...
  texto <- paste0(
    ...
    "\n",
    "Las concentraciones de TPH, en [ESCRIBIR_NUMERO_TEXTUAL] (", n_grillas_impactadas, ") áreas ubicada en ",
    "[ESCRIBIR_NUMERO_TEXTUAL] (", sprintf("%02d", n_locs_con_impactadas), ") locaciones (no supervisadas anteriormente) ",
    "correspondientes a los pozos: ", detalle_puntos_impactados, ", ",
    ...
    "es decir, un cuadrante de ", lado_grilla, " m de lado cuyo centroide tiene las coordenadas del punto de muestreo.\n",
    "\n",
    "Considerando que la conclusión, vista más adelante, contempla una jerarquía de Locación sobre Celda sobre Rejilla para la acusación de incumplimiento en la remediación, es decir, solo se sugiere la remediación de grillas que no pertenezcan a celdas completas impactadas ni locaciones completas impactadas; y, en la misma línea, solo se sugiere la remediación de celdas que no pertenezcan a locaciones completas impactadas; con el fin de evitar doble acusación. En consecuencia, a continuación, se presenta un cuadro que detalla el área de la parcela de rejillas cuyo punto muestreo superó el nivel de intervención de ",
    ...
    " mg/kg, se considera que existe evidencia técnica suficiente para calificar a la celda como impactada, debiendo ser sujeta a procesos de remediación en su totalidad.\n",
    "Proporción de puntos impactados: Se estimó la proporción de puntos de muestreo dentro del polígono de cada celda cuyos valores de TPH superan el umbral de ",
    format(umbral, big.mark = ",", decimal.mark = "."), 
    " mg/kg. Si esta proporción es mayor o igual al 50.0% respecto al total de puntos evaluados en la celda, también se considera que existe evidencia técnica suficiente para clasificarla como impactada, recomendando su remediación integral.\n",
    "\n",
    "Considerando que la conclusión, vista más adelante, contempla una jerarquía de Locación sobre Celda sobre Rejilla para la acusación de incumplimiento en la remediación, es decir, solo se sugiere la remediación de grillas que no pertenezcan a celdas completas impactadas ni locaciones completas impactadas; y, en la misma línea, solo se sugiere la remediación de celdas que no pertenezcan a locaciones completas impactadas; con el fin de evitar doble acusación. En consecuencia, a continuación, se presenta un cuadro que detalla el área de la parcela de celdas cuyo promedio de muestreo superó el nivel de intervención de ",
    format(umbral, big.mark = ",", decimal.mark = "."), " mg/kg establecido en el Estudio Ambiental del ", nombre_lote, 
    "o cuya proporción de puntos que supera dicho umbral está sobre el 50%, por locación supervisada:\n",
    "\n",
    "Tabla X2. Coordenadas de los vértices del polígono del área de celdas cuyo promedio ponderado o proporción de puntos impactados superó el Nivel de Intervención de ",
    format(umbral, big.mark = ",", decimal.mark = "."), " mg/kg establecido en el Estudio Ambiental del ", nombre_lote, "\n",
    "\n",
    ...
    format(umbral, big.mark = ",", decimal.mark = "."), 
    " mg/kg, se considera que existe evidencia técnica suficiente para calificar a la locación como impactada, debiendo ser sujeta a procesos de remediación en su totalidad.\n",
    "Proporción de puntos impactados: Se estimó la proporción de puntos de muestreo dentro de los polígonos de cada locación cuyos valores de TPH superan el umbral de ",
    format(umbral, big.mark = ",", decimal.mark = "."), 
    " mg/kg. Si esta proporción es mayor o igual al 50.0% respecto al total de puntos evaluados en la locación, también se considera que existe evidencia técnica suficiente para clasificarla como impactada, recomendando su remediación integral.\n",
    "\n",
        "Considerando que la conclusión, vista más adelante, contempla una jerarquía de Locación sobre Celda sobre Rejilla para la acusación de incumplimiento en la remediación, es decir, solo se sugiere la remediación de grillas que no pertenezcan a celdas completas impactadas ni locaciones completas impactadas; y, en la misma línea, solo se sugiere la remediación de celdas que no pertenezcan a locaciones completas impactadas; con el fin de evitar doble acusación. En consecuencia, a continuación, se presenta un cuadro que detalla el área de la parcela de todas las celdas de las locaciones cuyo promedio de muestreo superó el nivel de intervención de intervención ",
    format(umbral, big.mark = ",", decimal.mark = "."), " mg/kg establecido en el Estudio Ambiental del ", nombre_lote, 
    "o cuya proporción de puntos que supera dicho umbral está sobre el 50%, por locación supervisada:\n",
    "\n",
    "Tabla X3. Coordenadas de los vértices del polígono del área de celdas de las locaciones cuyo promedio ponderado o proporción de puntos impactados superó el Nivel de Intervención de ",
    format(umbral, big.mark = ",", decimal.mark = "."), " mg/kg establecido en el Estudio Ambiental del ", nombre_lote, "\n",
    "\n",
    ...
    "\n",
    "Prioridad de celdas sobre rejillas: De igual forma, si una celda ha sido clasificada como impactada y, por tanto, sujeta a remediación, esta determinación se antepone a los resultados de rejillas individuales contenidas dentro de dicha celda que también hayan sido calificadas como impactadas.\n",
    "\n",
    ...
    round(area_grillas_jerarquia, 2), " m² de las ", n_grillas_impactadas, 
    " descritas en el Cuadro N° 9: ", codigos_grillas_jerarquia,
    if (n_locaciones_impactadas > 0 && nchar(puntos_en_locs_impactadas) > 0) 
      paste0("; exceptuando las rejillas de los puntos de muestreo de la locación ", locs_impactadas_nombres, ": ", puntos_en_locs_impactadas, ".") 
    else ".", "\n",
    ...
    round(area_locaciones, 2), " m² descrita en el Cuadro N° 11: ", locs_impactadas_nombres, ".\n",
    "\n",
    ...
    "\n",
    nombre_empresa, " no cumplió con remediar [ESCRIBIR_NUMERO_TEXTUAL] (", n_grillas_impactadas, ") áreas ubicada en ",
    "[ESCRIBIR_NUMERO_TEXTUAL] (", sprintf("%02d", n_locs_con_impactadas), ") locaciones (no supervisadas anteriormente) ",
    "correspondientes a los pozos: ", detalle_puntos_impactados, ", ",
    "superan el nivel de intervención de ", format(umbral, big.mark = ",", decimal.mark = "."), 
    " mg/kg establecido en el Estudio Ambiental del ", nombre_lote, ".\n",
    ...
    " m² que constituye [ESCRIBIR_NUMERO_TEXTUAL] (", sprintf("%02d", n_grillas_jerarquia), ") rejillas de las ", n_grillas_impactadas, " descritas en el Cuadro N° 9; ",
    ...
    "[ESCRIBIR_NUMERO_TEXTUAL] (", sprintf("%02d", n_locaciones_impactadas), ") locación descrita en el Cuadro N°11. ",
    ...
    nombre_empresa, " no cumplió con remediar [ESCRIBIR_NUMERO_TEXTUAL] (", n_grillas_impactadas, ") áreas ubicada en ",
    "[ESCRIBIR_NUMERO_TEXTUAL] (", sprintf("%02d", n_locs_con_impactadas), ") locaciones (no supervisadas anteriormente) ",
    "correspondientes a los ", detalle_puntos_impactados, ".\n",
    "\n",
    "\n",
    "═══════════════════════════════════════════════════════════════════════\n",
    "NOTAS IMPORTANTES:\n",
